...
{
    output << "prints info about AmbisonicsDRIR SOFA files" << std::endl;
    output << "    syntax : ./AmbisonicsDRIRinfo [filename]" << std::endl;
}

/************************************************************************************/
/*!
 *  @brief          Helper function to  access element [i][j][k][l] of a "4D array" of dimensions [dim1][dim2][dim3][dim4]
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if( isADRIR == true )
        {
            output << filename << " is a valid 'AmbisonicsDRIR' file" << std::endl;
        }
        else
        {
            output << filename << " is not a valid 'AmbisonicsDRIR' file" << std::endl;
            return 0;
        }
        
        double sr = 0.0;
        const bool ok = adrir.GetSamplingRate( sr );
        
        SOFA_ASSERT( ok == true );
        
        sofa::Units::Type units;
        adrir.GetSamplingRateUnits( units );
        ...
        const unsigned int M = (unsigned int) adrir.GetNumMeasurements();
        const unsigned int R = (unsigned int) adrir.GetNumReceivers();
        const unsigned int E = (unsigned int) adrir.GetNumEmitters();
        const unsigned int N = (unsigned int) adrir.GetNumDataSamples();
            ...
            for( std::size_t i = 0; i < M; i++ )
            {
                for( std::size_t j = 0; j < R; j++ )
                {
                    for( std::size_t k = 0; k < E; k++ )
                    {
                        ...
                        {
                            const std::size_t index = array4DIndex( i, j, k, l, M, R, E, N );
